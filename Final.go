...
func tampilkanStatistik(data *[maxData]Sampah, count int) {
	...
	for i := 0; i < count; i++ {
		...
		t, _ := time.Parse("2006-01-02", entry.Tanggal)
		y, w := t.ISOWeek()
		if y == year && w == week {
...
func tampilkanRiwayat(data *[maxData]Sampah, count int) {
	...
	for _, s := range slice {
		fmt.Printf("%-15s %-8.1f %-12s %-10t\n", s.Jenis, s.Berat, s.Tanggal, s.Recyclable)
	}
}
...
func editData(data *[maxData]Sampah, currentIndex *int) {
	...
	for i := 0; i < *currentIndex; i++ {
		s := (*data)[i]
		fmt.Printf("%d. %s | %.1f kg | %s | Recyclable: %t\n", i+1, s.Jenis, s.Berat, s.Tanggal, s.Recyclable)
	}
